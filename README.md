...
Start the dataset-backed Flask API:
...
```bash
python -m http.server 5501 --directory code/frontend
```

Open `http://127.0.0.1:5501`. The onboarding screen uses a local demo OTP (`123456`); it does not connect to a bank or retrieve live financial data. After onboarding, purchase requests are sent to Flask and evaluated by the existing `BuyOrWaitEngine`.
